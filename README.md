...
- `/news` applies a presentation-only, hydration-safe gateway policy to the reviewed article set. The three general service guides remain visible. Six unreliable articles remain unchanged at the approved origin, are hidden from the rendered Legal Updates page, and their exact official-domain paths return temporary `307` redirects to `/news`.
